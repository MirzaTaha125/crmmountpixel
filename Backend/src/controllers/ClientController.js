...
export async function generateClientId(brand) {
  // Prefix comes from the Brand record's code (managed in the Brands tab) instead of
  // a hardcoded map. Existing client IDs are never regenerated, so they stay intact.
  if (!brand || !brand.trim()) return null;
  const brandDoc = await Brand.findOne({ name: brand.trim() });
  const prefix = brandDoc?.code?.trim();
  if (!prefix) {
    return null; // No client ID for brands without a code
  }

  // Find the highest existing client ID for this PREFIX (not the brand name). This keeps
  // the numbering continuous even if a brand's name differs slightly from older records
  // (e.g. "Webdeveloper Inc" vs "Webdevelopers Inc"), which would otherwise restart at
  // 001 and collide with an existing ID like WDI001.
  const escapedPrefix = prefix.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  const clients = await Client.find({
    clientId: { $regex: `^${escapedPrefix}[0-9]` }
  }).sort({ clientId: -1 }).limit(1);

  let nextNumber = 1;

  if (clients.length > 0 && clients[0].clientId) {
    // Extract the trailing number from the existing client ID (e.g. "WDI014" -> 14)
...
export async function createClient(req, res) {
  try {
    ...
    // Handle duplicate key errors — report the field that actually collided so a
    // clientId or stale-index clash isn't misreported as an email duplicate.
    if (error.code === 11000) {
      const field = Object.keys(error.keyPattern || error.keyValue || {})[0] || 'field';
      const value = error.keyValue ? error.keyValue[field] : undefined;
      const friendly = field === 'email'
        ? 'A client with this email already exists'
        : `Duplicate ${field}${value !== undefined ? ` "${value}"` : ''} — this value already exists`;
      return res.status(400).json({ message: 'Duplicate entry', error: friendly, field });
    }
...
export async function updateClient(req, res) {
  try {
    ...
    // Fetch existing client once — we may need it for amount calcs AND for
    // back-filling a clientId when a brand is added to a previously brand-less client.
    let existingClient = null;
    if (
      req.body.totalAmount !== undefined ||
      req.body.paidAmount !== undefined ||
      req.body.dueAmount !== undefined ||
      (updateData.brand && !updateData.clientId)
    ) {
      existingClient = await Client.findById(id);
    }

    // If the client previously had no brand-based clientId and we're now adding a brand,
    // generate one so it follows the same numbering as freshly-created clients of that brand.
    // (Skip if the caller explicitly provided a clientId to set.)
    if (updateData.brand && !updateData.clientId && existingClient && !existingClient.clientId) {
      try {
        const newClientId = await generateClientId(updateData.brand);
        if (newClientId) updateData.clientId = newClientId;
      } catch (e) {
        console.error('Failed to generate clientId on brand update:', e.message);
      }
    }
