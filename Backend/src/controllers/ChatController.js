...
export const sendMessage = async (req, res) => {
  try {
    const { chatId, message, clientId, attachments } = req.body;
    ...
    const lastMessagePreview = (message || '').trim() || (hasAttachments ? '📎 Attachment' : '');
...
export const editMessage = async (req, res) => {
  try {
    ...
    // Only the sender can edit (clients edit messages with no senderId; users edit messages they sent)
    const isOwn = isClient
      ? (!existing.senderId && existing.clientId && existing.clientId.toString() === userId.toString())
      : (existing.senderId && existing.senderId.toString() === userId.toString());
    ...
    if (existing.senderId) {
      populated = await Message.findById(existing._id)
        .populate('senderId', 'First_Name Last_Name Email Role workEmailName');
    } else {
      populated = existing.toObject();
    }

    try {
      const io = getIO();
      io.to(existing.chatId.toString()).emit('message_updated', populated);
    } catch (err) {
      console.error('Socket emit error:', err);
    }
...
export const deleteMessage = async (req, res) => {
  try {
    ...
    const isAdmin = !isClient && req.user?.Role === 'Admin';
    if (!isOwn && !isAdmin) {
      return res.status(403).json({ message: 'You can only delete your own messages' });
    }

    // Use findByIdAndUpdate so the `required: true` validator on `message`
    // doesn't reject our blanked-out fields.
    await Message.findByIdAndUpdate(existing._id, {
      $set: { isDeleted: true, message: '', attachments: [] },
    });

    try {
      const io = getIO();
      io.to(existing.chatId.toString()).emit('message_deleted', {
...
// Upload an attachment file for a chat message
...
// Get list of currently online user/client IDs
export const getOnlineUsers = async (req, res) => {
  try {
    res.status(200).json({ userIds: getOnlineUserIds() });
  } catch (error) {
