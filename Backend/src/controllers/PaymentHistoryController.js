...
export async function addPaymentRecord(req, res) {
  try {
    ...
    if (error.name === 'ValidationError') {
      const fieldErrors = Object.values(error.errors).map(e => e.message).join(', ');
      return res.status(400).json({ message: 'Validation error', error: fieldErrors });
    }
...
export async function updatePaymentRecord(req, res) {
  try {
    ...
    // Keep the linked Invoice record (Invoice Generator list) in sync with the
    // PaymentHistory status. Only reacts when the status actually changed so
    // unrelated edits (amount, description, etc.) don't clobber the invoice.
    // Only touches MANUAL invoices — PayPal/Stripe invoices are driven by
    // their provider webhooks and shouldn't be flipped from CRM edits.
    if (updateData.status && updateData.status !== payment.status && payment.invoiceNumber) {
      try {
        ...
        if (invoice) {
          if (updateData.status === 'Completed') {
            ...
            invoice.paidAt = null;
          } else if (updateData.status === 'Failed' || updateData.status === 'Refunded') {
            // Map failed/refunded payments to the invoice's Refunded state so
            // the Invoice Generator list reflects that money didn't stay put.
        ...
        console.error('Error syncing linked Invoice status:', invErr);
        // Don't fail the request if the invoice sync fails.
      }
    }
